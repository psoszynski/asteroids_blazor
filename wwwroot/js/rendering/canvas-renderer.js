// Power-up glyphs are drawn inline (rather than imported from
// powerup-icons.js) so this eagerly-bundled module never depends on a
// separate network chunk; three-renderer.js (lazy-loaded) shares the
// canonical implementation via that module instead. Keep the two in sync
// if the glyphs change.
 ...
 */
function drawPowerUpIcon(ctx, type, size) {
    switch (type) {
        case 1:
            boltPath(ctx, size);
            ctx.fill();
...
export function createCanvasRenderer() {
    ...
    const POWERUP_COLORS = {
        0: { fill: '#5ce8ff', glow: '#2ab8ff' },
        1: { fill: '#ffb347', glow: '#ff6a00' },
        2: { fill: '#d98cff', glow: '#b44dff' }
    };
    ...
    const drawPowerUps = (ctx, powerUps, time) => {
        for (const powerUp of powerUps) {
            ...
            drawPowerUpIcon(ctx, powerUp.type, radius * 0.72);
            ctx.restore();
        }
    };
